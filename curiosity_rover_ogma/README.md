...
The property in question is listed in the file `manifest/document.json`, which
lists the following property, where `input_signal` represents the rover's X
coordinate:
...
To run the demo, we need at least 3 terminals.
...
In the same terminal, start the monitoring node from this specific demo's
directory.

```sh
cd ../curiosity_rover_ogma
./run.sh
```
...
the rover's odometry's pose becomes greater than 2.5), Terminal 2 will start
